...
// Requests go through a self-hosted Fastify proxy on Fly.io (Sao Paulo, gru).
...
//
// Uses the service role (admin) client: this is a background service operation
// with no user session context. RLS enforcement is handled at the application
// layer -- the calling Server Component has already verified authentication.
...
async function buildAndPersistProfile(
  ...
  // Service role bypasses RLS intentionally: bcraService runs as a background
  // data pipeline without a user session. The authenticated gate is enforced
  // upstream in the Server Component that calls this service.
  ...
  if (error) {
    console.error("[BCRA] Profile upsert failed:", error.message);
    // Return un-persisted data so the query renders; next request re-fetches
    // from BCRA since nothing was cached.
    return {
...
// Cache read uses the service role client: profiles are written by the service
// pipeline (no user session) so reads follow the same trust boundary.
...
export async function fetchBcraReportByDni(rawDni: string): Promise<BcraProfileResult | null> {
  ...
  for (const prefix of DNI_PREFIXES) {
    ...
    if (!cuil) {
      console.log(`[BCRA] Prefijo ${prefix}+${paddedDni} -> digito verificador invalido (rem=1), saltando`);
      continue;
    }
    ...
    try {
      ...
      console.log(`[BCRA] ERROR de red en /Deudas/${cuil}: ${describeError(err)} -- abortando busqueda`);
      throw err;
    }
    ...
    if (!deuda) {
      ...
      if (nextPrefix) {
        console.log(`[BCRA] Fallo ${cuil}, probando prefijo ${nextPrefix}...`);
      }
      continue;
    }
  ...
  console.log(`[BCRA] Busqueda agotada para DNI ${paddedDni}. Verificado en ANSES/BCRA manualmente?`);
  return null;
}
